...
exports.sendPushNotification = functions.database.ref('users/{id}/infant').onUpdate(event => {
  ...
  var messages = [];   //Create an empty array to push stuff into

    if (snapshot.val() === true) {   //If infant is true/ changed to true
      return ref.once('value').then((snapshot) => {
        console.log(snapshot);
        var expoToken = snapshot.val().expoToken; //fetch expotoken (credential for push notification)
              messages.push({
                  "to": expoToken,
                  "sound": "default",
...
exports.sendWeeklySMS = functions.https.onRequest((req, res) => {
  ...
  let today = new Date();
  let date = (today.getMonth() + 1).toString().padStart(2, "0") + '/' + today.getDate().toString().padStart(2, "0") + '/' + today.getFullYear(); //Parse todays date to a string
  ref.orderByChild("nextWeek").equalTo(date).once("value") //filters the users for which value of "nextWeek" matches "date"
  .then((snapshot) => {
    snapshot.forEach((childSnapshot) => {  //loops through said filtered users
      ...
      let motherMsg = getLocalizedText.translate(deviceLanguage, week, "Mother") //fetch mother message
      ...
      childSnapshot.ref.update({   //Update the database with the updated values
        nextWeek: nextWeek,
        week: nextWeekNo
      });
    })
...
exports.sendAppointmentReminder = functions.https.onRequest((req, res) => {
  ...
  let date = (tomorrow.getMonth()+1).toString().padStart(2, "0") +'/'+tomorrow.getDate().toString().padStart(2, "0") +'/'+ tomorrow.getFullYear(); //This converts the date to a string in the format ["MM/DD/YYYY"]
  ref.once("value")
  .then((snapshot) => {   //This is the way to activate the function to do stuff in the database
    snapshot.forEach((childSnapshot) => {  //childSnapshot in this case is each individual user
      ...
      appointmentSnapshot.forEach( childAptmtSnapshot => {   //Loops through each Appointment
        let aptmtDate = childAptmtSnapshot.child("date").val();
        if (aptmtDate === date) {   //If appt date is tomorrow send reminder
