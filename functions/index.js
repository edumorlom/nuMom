  ...
  .onUpdate((event) => {
    ...
    if (snapshot.val() === true) {
      ...
      return ref.once('value').then((snapshot) => {
        ...
        const {expoToken} = snapshot.val(); // fetch expotoken (credential for push notification)
        messages.push({
          to: expoToken,
          sound: 'default',
          body:
  ...
  ref
    ...
    .then((snapshot) => {
      snapshot.forEach((childSnapshot) => {
        ...
        const {deviceLanguage} = childSnapshot.val();
        ...
        const babyMsg = getLocalizedText.translate(
          deviceLanguage,
          week,
          'Baby'
        ); // fetch baby message
